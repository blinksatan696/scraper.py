...
def fetch_html(url, market_name):
    """Mengambil HTML dengan strategi anti-blokir dan anti-timeout"""
    ...
    try:
        with sync_playwright() as p:
            ...
            # PERBAIKAN: Gunakan domcontentloaded agar tidak timeout menunggu iklan
            page.goto(url, wait_until="domcontentloaded", timeout=30000)
            time.sleep(3)  # Jeda singkat agar JS selesai merender tabel
        ...
        print(f"   ⚠️ Error koneksi Playwright: {e}")
        # Fallback ke urllib jika Playwright gagal total
        try:
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'})
            with urllib.request.urlopen(req, timeout=15) as response:
                return response.read().decode('utf-8')
        except Exception as e2:
            print(f"   ⚠️ Error koneksi Fallback: {e2}")
            return None
...
def parse_california(html_text, _):
    ...
    for match in re.finditer(r'(\d{1,2})/(\d{1,2})/(\d{4})', text):
        m, d, y = match.groups()
        formatted_date = f"{int(d):02d}-{int(m):02d}-{y}"
        snippet = text[match.end():match.end()+400]  # Diperluas untuk menangkap angka
        
        # Cari 4 digit berturut-turut
        digits = re.findall(r'\b(\d{4})\b', snippet)
        for nomor in digits:
            if not any(r['tanggal'] == formatted_date and r['nomor'] == nomor for r in results):
                results.append({"tanggal": formatted_date, "nomor": nomor})
                break  # Ambil satu saja per tanggal
    return results
